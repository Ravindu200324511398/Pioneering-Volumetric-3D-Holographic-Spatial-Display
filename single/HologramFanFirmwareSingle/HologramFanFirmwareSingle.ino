...
// ---------------------------------------------------------
// HARDWARE CONFIGURATION (Single Fan Mode)
// ---------------------------------------------------------
...
void handleUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    // SECURITY: write to the fixed single-fan bin path
    String path = "/fan.bin";
    ...
    if (uploadFile) {
      uploadFile.close();
      // Verify the number of bytes actually written matches expected
...
void setup() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\n\n=== Hologram Fan Firmware (Single Fan Mode) ===");
  ...
  // Setup Wi-Fi AP
  WiFi.softAP("HologramFan", "12345678");
  Serial.print("WiFi AP started! IP: ");
  Serial.println(WiFi.softAPIP());

  // Disable Wi-Fi power-save (modem sleep) to eliminate POV jitter
  WiFi.setSleep(false);
...
void loop() {
  ...
  if (newRotation && imageLoaded) {
    ...
    // Atomically snapshot both timestamps together
    noInterrupts();
    ...
    if (rotationUs > 8000 && rotationUs < 500000) {
      ...
      for (int row = 0; row < POLAR_ROWS; row++) {
        ...
        // Overrun check to prevent rotation/color drift
      ...
      if (overrunCount > 0) {
        Serial.printf("WARNING: dropped %lu/%d rows this rotation (render fell behind)\n",
